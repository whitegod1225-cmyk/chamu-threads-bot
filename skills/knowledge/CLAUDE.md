...
STEP0 投稿目的を先に決める（必須）
投稿を書く前に、この投稿が**リーチ狙い**か**エンゲージ狙い**かを決める。目的によって型・時間帯・ツリー本数の設計がすべて変わる。

| 目的 | 型 | 時間帯 | ツリー |
|-----|-----|--------|-------|
| **リーチ**（新規流入・閲覧数） | 第三者権威型（型14） | 朝 | 4本立て |
| **エンゲージ**（いいね・保存・コメント） | 定義ずらし型（型②概念再定義）・保存版型 | 夜〜深夜 | 1本立て |

- 引数で明示されていない場合は「エンゲージ狙い」をデフォルトにする
- 決定した目的をSTEP3の型選択に反映する
